...
with open('pop_ingredients_in_clusters.pickle', 'rb') as handle:
    pop_ingredients_in_cluster = pickle.load(handle)

# Constants
NUMBER_OF_TOP_INGREDIENTS = 5
...
for key, value in pop_ingredients_in_cluster.items():
    ...
    for i in range(NUMBER_OF_TOP_INGREDIENTS):
        ingredients_matrix[key][i] = map_[value[i][0]]
        count_matrix[key][i] = value[i][1]
